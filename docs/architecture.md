...
Dashboard Auth persists in tab-scoped sessionStorage, validates identity with `getUser`, and checks membership before querying candidates. Token changes or hidden tabs clear displayed data; returning to the tab revalidates. RLS, not the page's visibility, enforces security. Supabase tokens are still accessible to JavaScript: keep third-party scripts out, use textContent for untrusted values, and do not render supplier HTML without sanitization.
